-- The function's own RETURNS TABLE(application_id, applicant_id) columns
-- shadow the applications table's columns of the same name inside the
-- function body. The duplicate-application EXISTS check used unqualified
-- `applicant_id`/`job_posting_id`, which Postgres rejected as ambiguous
-- (42702) — this only fires on the "existing applicant" branch, so a first
-- ever application for a given email never hit it, only a genuine repeat.
